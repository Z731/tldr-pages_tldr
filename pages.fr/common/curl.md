...
`curl {{https://exemple.fr}} --output {{nom_fichier}}`

- Télécharger le contenu d'une URL dans un fichier nommé comme indiqué par l'URL :

`curl --remote-name {{https://exemple.fr/nom_fichier}}`
...
`curl --fail --remote-name --location --continue-at - {{https://exemple.fr/nom_fichier}}`
