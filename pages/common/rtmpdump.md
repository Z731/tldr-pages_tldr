...
`rtmpdump --rtmp {{rtmp://example.com/path/to/video}} --swfVfy {{https://example.com/player}} --flashVer "{{LNX 10,0,32,18}}" -o {{file.ext}}`
...
`rtmpdump --rtmp {{rtmp://example.com/path/to/video}} --pageUrl {{https://example.com/webpage}} -o {{file.ext}}`
